...
antlrcpp::Any TypeCheckVisitor::visitMap(AslParser::MapContext *ctx) {
  ...
  if (
      (not Types.isErrorTy(t1) and not Types.isArrayTy(t1)) or
      (not Types.isErrorTy(t2) and not Types.isArrayTy(t2)) or
      (Types.isArrayTy(t1) and Types.isArrayTy(t2) and Types.getArraySize(t1) != Types.getArraySize(t2)) or
      (not Types.isErrorTy(t3) and not Types.isFunctionTy(t3)) or 
      (Types.isFunctionTy(t3) and 
        (not Types.equalTypes(Types.getParameterType(t3,0), Types.getArrayElemType(t1))
        or not Types.equalTypes(Types.getFuncReturnType(t3), Types.getArrayElemType(t2))
        )
      )
      )
...
antlrcpp::Any TypeCheckVisitor:: visitPow(AslParser::PowContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->expr(0));
  visit(ctx->expr(1));
  TypesMgr::TypeId t1 = getTypeDecor(ctx->expr(0));
  ...
  //cout<<"This is t2: " + t2.to_string()<<endl;
  //std::cout << "t2 " << Types.to_string_basic(t2) << "\n";
  if ((not Types.isErrorTy(t1) and not Types.isNumericTy(t1)) or (not Types.isErrorTy(t2) and not Types.isIntegerTy(t2)))
    Errors.incompatibleOperator(ctx->op);
...
antlrcpp::Any TypeCheckVisitor::visitFunction_call(AslParser::Function_callContext *ctx) {
  ...
  TypesMgr::TypeId tFunc = Types.createErrorTy();   //retorno de la funcion


  if (not Types.isFunctionTy(tID) and not Types.isErrorTy(tID))   // no da error pero no es tipo funcion 
    Errors.isNotCallable(ctx->ident());
  ...
  if (Types.isFunctionTy(tID)) {
    ...
    else tFunc = Types.getFuncReturnType(tID);  // no es void, cogemos su return type
